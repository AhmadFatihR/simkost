<div class="container-fluid">
        <!--  Row 1 -->
            <div class="row row-cols-1 row-cols-md-3">

              <div class="col">
                <!-- Monthly Earnings -->
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-start">
                      <div class="col-8">
                        <h5 class="card-title mb-9 fw-semibold"> Penghuni Kost</h5>
                        <h4 class="fw-semibold mb-3">3</h4>
                      </div>
                      <div class="col-4">
                        <div class="d-flex justify-content-end">
                          <div
                            class="text-white bg-primary rounded-circle p-6 d-flex align-items-center justify-content-center">
                            <i class="ti ti-users fs-8"></i>
                          </div>
                        </div>
                      </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div class="col">
                <!-- Monthly Earnings -->
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-start">
                      <div class="col-8">
                        <h5 class="card-title mb-9 fw-semibold"> Kamar Kost </h5>
                        <h4 class="fw-semibold mb-3">8</h4>
                      </div>
                      <div class="col-4">
                        <div class="d-flex justify-content-end">
                          <div
                            class="text-white bg-secondary rounded-circle p-6 d-flex align-items-center justify-content-center">
                            <i class="ti ti-home fs-8"></i>
                          </div>
                        </div>
                      </div>
                    </div>
                    </div>
                  </div>
                </div>

                <div class="col">
                <!-- Monthly Earnings -->
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-start">
                      <div class="col-8">
                        <h5 class="card-title mb-9 fw-semibold"> Jumlah Komplain </h5>
                        <h4 class="fw-semibold mb-3">2</h4>
                      </div>
                      <div class="col-4">
                        <div class="d-flex justify-content-end">
                          <div
                            class="text-white bg-danger rounded-circle p-6 d-flex align-items-center justify-content-center">
                            <i class="ti ti-bell fs-8"></i>
                          </div>
                        </div>
                      </div>
                    </div>
                    </div>
                  </div>
                </div>
            </div>
                
                <div class="row row-cols-1 row-cols-md-2">
                <div class="col">
                <!-- Monthly Earnings -->
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-start">
                      <div class="col-8">
                        <h5 class="card-title mb-9 fw-semibold"> Jumlah Tagihan </h5>
                        <h4 class="fw-semibold mb-3">3</h4>
                      </div>
                      <div class="col-4">
                        <div class="d-flex justify-content-end">
                          <div
                            class="text-white bg-warning rounded-circle p-6 d-flex align-items-center justify-content-center">
                            <i class="ti ti-notes fs-8"></i>
                          </div>
                        </div>
                      </div>
                    </div>
                    </div>
                  </div>
                </div>

                <div class="col">
                <!-- Monthly Earnings -->
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-start">
                      <div class="col-8">
                        <h5 class="card-title mb-9 fw-semibold"> Total Pendapatan </h5>
                        <h4 class="fw-semibold mb-3">Rp 100.000</h4>
                      </div>
                      <div class="col-4">
                        <div class="d-flex justify-content-end">
                          <div
                            class="text-white bg-success rounded-circle p-6 d-flex align-items-center justify-content-center">
                            <i class="ti ti-cash fs-8"></i>
                          </div>
                        </div>
                      </div>
                    </div>
                    </div>
                  </div>
                </div>
                </div>

                <div class="card mb-3 text-center">
                  <img src="../assets/images/backgrounds/tes.png" class="card-img-top" alt="...">
                    <div class="card-body">
                    <h5 class="card-title">Selamat datang di aplikasi SIMKOST.</h5>
                    <p class="card-text"> Aplikasi ini memudahkan Anda dalam melihat history transaksi dan melaporkan kendala terkait fasilitas kost.</p>
                    </div>
                  </div>

            </div>
          </div>
        </div>
      </div>